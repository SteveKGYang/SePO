...
from myopenrlhf.datasets import RewardDataset
from myopenrlhf.models import Actor
from myopenrlhf.trainer import DPOTrainer
...
def train(args):
    ...
    # )
    full_dataset = load_dataset(args.dataset, split='train_prefs')
    prob = 0.8
    # train_num = int(len(full_dataset)*prob)
    # train_data = full_dataset.select(range(4000, train_num))
    train_data = full_dataset.select(range(4000, len(full_dataset)))
    eval_data = full_dataset.select(range(0, 4000))
